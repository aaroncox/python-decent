...
class Account(dict):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises decent.exceptions.AccountDoesNotExistsException: if account does not exist
    ...
    def refresh(self):
        ...
        if re.match("^1\.2\.[0-9]*$", self.name):
            account = self.decent.rpc.get_objects([self.name])[0]
        else:
            account = self.decent.rpc.lookup_account_names([self.name])[0]
        if not account:
            raise AccountDoesNotExistsException(self.name)

        if self.full:
            account = self.decent.rpc.get_full_accounts([account["id"]], False)[0][1]
    ...
    def balances(self):
        """ List balances of an account. This call returns instances of
            :class:`decent.amount.Amount`.
        """
        balances = self.decent.rpc.get_account_balances(self["id"], [])
        return [
            Amount(b, decent_instance=self.decent)
    ...
    def history(
        ...
        mostrecent = self.decent.rpc.get_account_history(
            self["id"],
            "1.11.{}".format(0),
            1,
        ...
        while True:
            # RPC call
            txs = self.decent.rpc.get_account_history(
                self["id"],
                "1.11.{}".format(last),
                _limit,
                "1.11.{}".format(first - 1),
    ...
    def upgrade(self):
        return self.decent.upgrade_account(account=self)


class AccountUpdate(dict):
    """ This purpose of this class is to keep track of account updates
        as they are pushed through by :class:`decent.notify.Notify`.
    ...
    def __init__(
        ...
        if isinstance(data, dict):
            ...
            account = Account(data, decent_instance=self.decent)
            update = self.decent.rpc.get_objects([
    ...
    def account(self):
        """ In oder to obtain the actual
            :class:`decent.account.Account` from this class, you can
