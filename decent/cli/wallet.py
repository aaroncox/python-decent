...
def changewalletpassphrase(ctx, new_password):
    ...
    """
    ctx.decent.wallet.changePassphrase(new_password)
    ...
    if not key:
        while True:
            ...
            if not key or key == "exit":
                break
            try:
                ctx.decent.wallet.addPrivateKey(key)
        ...
        for k in key:
            try:
                ctx.decent.wallet.addPrivateKey(k)
            except Exception as e:
                click.echo(str(e))

    installedKeys = ctx.decent.wallet.getPublicKeys()
    if len(installedKeys) == 1:
        name = ctx.decent.wallet.getAccountFromPublicKey(installedKeys[0])
        account = Account(name, decent_instance=ctx.decent)
        click.echo("=" * 30)
...
def delkey(ctx, pubkeys):
    ...
    if click.confirm(
        ...
        for pub in pubkeys:
            ctx.decent.wallet.removePrivateKeyFromPublicKey(pub)
...
def getkey(ctx, pubkey):
    ...
    """
    click.echo(ctx.decent.wallet.getPrivateKeyForPublicKey(pubkey))
...
def listkeys(ctx):
    ...
    t.align = "l"
    for key in ctx.decent.wallet.getPublicKeys():
        t.add_row([key])
    click.echo(t)
...
def listaccounts(ctx):
    ...
    t.align = "l"
    for account in ctx.decent.wallet.getAccounts():
        t.add_row([
            account["name"] or "n/a",
            account["type"] or "n/a",
            account["pubkey"]
