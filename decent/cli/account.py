...
def history(ctx, account, limit, type, csv, exclude, raw):
    ...
    """
    from decentbase.operations import getOperationNameForId
    ...
    for a in account:
        account = Account(a, decent_instance=ctx.decent)
...
def newaccount(ctx, accountname, account, password):
    ...
    """
    pprint(ctx.decent.create_account(
        accountname,
        registrar=account,
        password=password,
    ))
...
def upgrade(ctx, account):
    pprint(ctx.decent.upgrade_account(account))
