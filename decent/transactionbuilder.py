...
from decentbase.objects import Operation
from decentbase.account import PrivateKey, PublicKey
from decentbase.signedtransactions import Signed_Transaction
...
class TransactionBuilder(dict):
    ...
    def __init__(self, tx={}, decent_instance=None):
    ...
    def appendSigner(self, account, permission):
        ...
        account = Account(account, decent_instance=self.decent)
        ...
        def fetchkeys(account, level=0):
            ...
            for authority in account[permission]["key_auths"]:
                wif = self.decent.wallet.getPrivateKeyForPublicKey(authority[0])
                if wif:
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(authority[0], decent_instance=self.decent)
    ...
    def constructTx(self):
        ...
        if self.decent.proposer:
            ...
            ops = operations.Proposal_create(**{
                ...
                "expiration_time": transactions.formatTimeFromNow(
                    self.decent.proposal_expiration),
        ...
        ops = transactions.addRequiredFees(self.decent.rpc, ops)
        expiration = transactions.formatTimeFromNow(self.decent.expiration)
        ref_block_num, ref_block_prefix = transactions.getBlockParams(self.decent.rpc)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.decent.rpc:
            operations.default_prefix = self.decent.rpc.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.decent.rpc.chain_params)
    ...
    def verify_authority(self):
        ...
        """
        try:
            if not self.decent.rpc.verify_authority(self.json()):
    ...
    def broadcast(self):
        ...
        if self.decent.nobroadcast:
            log.warning("Not broadcasting anything!")
            return self

        # Broadcast
        try:
            self.decent.rpc.broadcast_transaction(self.json(), api="network_broadcast")
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        self["blockchain"] = self.decent.rpc.chain_params
    ...
    def appendMissingSignatures(self):
        ...
        for pub in missing_signatures:
            wif = self.decent.wallet.getPrivateKeyForPublicKey(pub)
            if wif:
                self.appendWif(wif)
