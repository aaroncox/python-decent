...
class Decent(object):
    """ Connect to the decent network.
        ...
        * **Wallet Database**: Here, the decentlibs load the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call ``decent()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``decent()``.
        * **Force keys**: This more is for advanced users and
        ...
        http://DCT-node.bitshares.eu. It is **highly** recommended that you pick your own
        node instead. Default settings can be changed with:

        .. code-block:: python

            decent set node <host>
        ...
        All that is requires is for the user to have added a key with ``decent``
        ...
        .. code-block:: bash
            ...
            decent set default_account xeroc
    ...
    def __init__(self,
        ...
        self.txbuffer = TransactionBuilder(decent_instance=self)
    ...
    def sign(self, tx=None, wifs=[]):
        ...
        """
        if tx:
            txbuffer = TransactionBuilder(tx, decent_instance=self)
        else:
    ...
    def broadcast(self, tx=None):
        """ Broadcast a transaction to the decent network
    ...
    def create_account(
        ...
        if not registrar:
            raise ValueError(
                ...
                "registrar=x, or set the default_account using 'decent'")
        ...
        try:
            Account(account_name, decent_instance=self)
            raise AccountExistsException
        except:
            pass

        referrer = Account(referrer, decent_instance=self)
        registrar = Account(registrar, decent_instance=self)

        " Generate new keys from password"
        from decentbase.account import PasswordKey, PublicKey
        if password:
        ...
        for k in additional_owner_accounts:
            addaccount = Account(k, decent_instance=self)
        ...
        # voting account
        voting_account = Account(proxy_account or "proxy-to-self", decent_instance=self)
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        ...
        account = Account(account, decent_instance=self)
        amount = Amount(amount, asset, decent_instance=self)
        to = Account(to, decent_instance=self)

        memoObj = Memo(
            from_account=account,
            to_account=to,
            decent_instance=self
        )
    ...
    def allow(self, foreign, weight=None, permission="active",
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, decent_instance=self)
    ...
    def approvewitness(self, witnesses, account=None):
        ...
        for witness in witnesses:
            witness = Witness(witness, decent_instance=self)
    ...
    def approvecommittee(self, committees, account=None):
        ...
        for committee in committees:
            committee = Committee(committee, decent_instance=self)
    ...
    def cancel(self, orderNumber, account=None):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=False, decent_instance=self)
    ...
    def approveproposal(self, proposal_ids, account=None, approver=None):
        ...
        for proposal_id in proposal_ids:
            proposal = Proposal(proposal_id, decent_instance=self)
    ...
    def betting_market_create(
        ...
        """ Create an event group. This needs to be **proposed**.
            ...
            :param decent.asset.Asset asset: Asset to be used for this market
