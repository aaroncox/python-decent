...
class Witness(dict):
    ...
    def refresh(self):
        ...
        if len(parts) == 3:
            ...
            if int(b) == 6:
                witness = self.decent.rpc.get_object(self.witness)
            else:
                witness = self.decent.rpc.get_witness_by_account(self.witness)
        else:
            account = Account(self.witness)
            witness = self.decent.rpc.get_witness_by_account(account["id"])
        if not witness:
...
class Witnesses(list):
    """ Obtain a list of **active** witnesses and the current schedule
        ...
        :param decent decent_instance: decent() instance to use when accesing a RPC
    """
    def __init__(self, decent_instance=None):
        self.decent = decent_instance or shared_decent_instance()
        self.schedule = self.decent.rpc.get_object("2.12.0").get("current_shuffled_witnesses", [])

        super(Witnesses, self).__init__(
            [
                Witness(x, lazy=True, decent_instance=self.decent)
